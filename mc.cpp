...
void abstract_mc::_do_measurement() {
	...
	measure.add("_ll_measurement_time", (tend.tv_sec-tstart.tv_sec)+1e-9*(tend.tv_nsec-tstart.tv_nsec));
}
...
void abstract_mc::_do_update() {
	...
	do_update();
	clock_gettime(CLOCK_MONOTONIC_RAW, &tend);
	measure.add("_ll_sweep_time", (tend.tv_sec-tstart.tv_sec)+1e-9*(tend.tv_nsec-tstart.tv_nsec));
}
...
void abstract_mc::_write(const std::string& dir) {
	...
	clock_gettime(CLOCK_MONOTONIC_RAW, &tend);
	measure.add("_ll_checkpoint_write_time", (tend.tv_sec-tstart.tv_sec)+1e-9*(tend.tv_nsec-tstart.tv_nsec));
}
...
bool abstract_mc::_read(const std::string& dir) {
	try {
		...
		clock_gettime(CLOCK_MONOTONIC_RAW, &tend);
		measure.add("_ll_checkpoint_read_time", (tend.tv_sec-tstart.tv_sec)+1e-9*(tend.tv_nsec-tstart.tv_nsec));
